...
void stColorGradient(
      const cv::Mat & imColor
    , cv::Mat & imGradient
)
{
    // Compute the Color Gradient Image
    // ********************************************************************** //
    ...
    cv::minMaxLoc(
          imGradient    // Array to search for extremes in
    ...
    {
        ptrGrad[i] = (
            (float)(10000.0 * ((double)(ptrGrad[i]) - minGrad)/(maxGrad - minGrad))
            + 1.0f
        );
    }
    // ********************************************************************** //
